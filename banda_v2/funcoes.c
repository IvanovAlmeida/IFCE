...
void cadastrarBandas(Banda b[TAM]){
	...
	fwrite(b, sizeof(Banda), TAM, arquivo);
}
...
void imprimirBandas(Banda b[TAM]){
	...
	fread(b, sizeof(Banda), TAM, arquivo);
 ...
 * @return void
 */
//void modificarBanda(Banda *b){
void modificarBanda(int pos){
	clear();
	int op, loop = 1;
    Banda b;

    fseek(arquivo, (sizeof(Banda)*(pos)), SEEK_SET);
	...
	fseek(arquivo, (sizeof(Banda)*pos), SEEK_SET);
	if( (fwrite(&b, sizeof(Banda), 1, arquivo)) == 1 )
